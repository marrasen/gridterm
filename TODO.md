...
A file dropped on a pane lands in the directory the shell said it was
in, and nothing is typed. A pane in WSL gets it on the share Windows
serves that distribution's files on.

- **A file already in that directory is left alone**, with a message
  saying so. Copying a file onto itself leaves nothing behind.

- **A shell that has not said where it is falls back to typing the
