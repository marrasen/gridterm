// Package syntax colours lines of a file by what kind of file it is:
// comments, strings, numbers and keywords in code, headings and
// bullets in Markdown, keys and values in JSON. It names colours by
...
const (
	// Plain is the reader's ordinary foreground.
	Plain Colour = iota

	// Note is a comment, or anything else beside the point.
	Note

	// Text is a string, a heading's text, or a quote.
	Text

	// Mark is punctuation that carries meaning: a heading's
	// hashes, a bullet, a number. The log view uses it for a warning.
	Mark

	// Bad is a failure: the level of a log line that went wrong.
	//
...
// For picks how a file is drawn, by what it is called, and
// gives nil for a name it does not know.
func For(name string) Colourer {
	...
	return func(line string, into []Run) []Run {
		...
		for i := 0; i < len(line); {
			if comments.begins(line, i) {
				out = add(out, at, i, Plain, 0)
				return add(out, i, len(line), Note, 0)
			}
			if q := line[i]; q == '"' || q == '\'' || q == '`' {
			...
			if isDigit(line[i]) && (i == 0 || !wordish(line[i-1]) && !inRune(line[i-1])) {
				...
				out = add(out, at, i, Plain, 0)
				out = add(out, i, end, Mark, 0)
				i, at = end, end
				continue
			}
			i++
		}
		return add(out, at, len(line), Plain, 0)
	}
}
...
func markdownRuns(line string, into []Run) []Run {
	...
	case heading(trimmed) > 0:
		...
		n := heading(trimmed)
		out = add(out, 0, indent+n, Mark, 0)
		return add(out, indent+n, len(line), Text, grid.AttrBold)
	case strings.HasPrefix(trimmed, ">"):
		return add(out, 0, len(line), Note, grid.AttrItalic)
	case strings.HasPrefix(trimmed, "```"), strings.HasPrefix(trimmed, "~~~"):
		return add(out, 0, len(line), Mark, 0)
	case isRule(trimmed):
		return add(out, 0, len(line), Mark, 0)
	default:
	...
	for i := from; i < len(line); {
		...
		case strings.HasPrefix(line[i:], "**"), strings.HasPrefix(line[i:], "__"):
			if end := marked(line, i, line[i:i+2]); end > i && opens(line, i) {
				out = add(out, at, i, Plain, 0)
				out = add(out, i, end, Plain, grid.AttrBold)
				i, at = end, end
				continue
			}
		case line[i] == '*', line[i] == '_':
			if end := marked(line, i, line[i:i+1]); end > i && opens(line, i) {
				out = add(out, at, i, Plain, 0)
				out = add(out, i, end, Plain, grid.AttrItalic)
				i, at = end, end
				continue
			}
		}
...
func add(out []Run, from, to int, col Colour, attr grid.Attr) []Run {
	...
	if n := len(out); n > 0 && out[n-1].Colour == col && out[n-1].Attr == attr {
		out[n-1].End = to
		return out
	}
	return append(out, Run{End: to, Colour: col, Attr: attr})
}
