...
// Arms says which sides a line-drawing character reaches out of its cell
// on, and whether it is one at all.
//
...
// comes apart; the renderer carries each arm across the gap on its own
// side.
func Arms(r rune) (left, right, up, down, ok bool) {
	bl, ok := boxLines[r]
	if !ok {
		return false, false, false, false, false
	}
	return bl.left != wNone, bl.right != wNone, bl.up != wNone, bl.down != wNone, true
}
