...
type app struct {
	...
	// servePanes is the pane open on what this window is serving, and
	// holds at most one: there is one thing being served.
	servePanes []*servingPane
...
func (a *app) commands() {
	...
	cmds.MustRegister(a.reportingAll([]ui.Command{
		...
		ui.Command{ID: "sshkey.make", Title: makeKeyTitle + "…",
			...
			Run:      a.openMakeKey},
		ui.Command{ID: installKeyCommand, Title: installKeyTitle + "…",
			AlsoFind: []string{"authorized_keys", "ssh-copy-id", "copy", "public key", "server", "add"},
			Run:      a.installKeyHere},
