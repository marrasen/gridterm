...
func (a *app) commands() {
	...
	cmds.MustRegister(a.reportingAll([]ui.Command{
		...
		ui.Command{ID: "secrets.addNote", Title: "Add a note…",
			...
			Run:      a.addNote},
		ui.Command{ID: "secrets.change", Title: "Change a secret…",
			AlsoFind: []string{"password", "vault", "note", "edit", "rename"},
			Run:      a.changeSecret},
		ui.Command{ID: "secrets.addKey", Title: "Let another key open the secrets…",
			AlsoFind: []string{"vault", "password", "machine", "key"},
			Run:      a.addVaultKey},
		ui.Command{ID: "secrets.removeKey", Title: "Stop a key opening the secrets…",
			AlsoFind: []string{"vault", "password", "machine", "key", "revoke"},
